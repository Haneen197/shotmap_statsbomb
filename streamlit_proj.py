...
#print(df.head) to make sure it read properly

df = df[df["type"] == 'Shot'].reset_index(drop=True) #show shots only
df["location"] = df["location"].apply(json.loads) #loads into list obj

team = st.selectbox('Select a team', df["team"].sort_values().unique(), index = None) #none index to avoid default selection
